    ...
    public static Command selectionRequest() {
        if (!User.getInstance().isRegistrationStatus()) {
            ...
            if (User.getInstance().isCreateNewUser()) {
                return new Command(Client.getUserUUID(), true, false, User.getInstance().getName(),
                        User.getInstance().getPassword(), "creating new user");
            }
            else {return new Command(Client.getUserUUID(),false, false, User.getInstance().getName(),
    ...
    public static Command enterCommand() {
        try {
            ...
            if (inputData.split(" ").length == 2) {
                inputCommand = new Command(Client.getUserUUID(),true, User.getInstance().getName(),
            ...
            if (Command.checkCommand(inputCommand.getNameOfCommand())) {
                if (Arrays.stream(new String[]{"add", "add_if_max", "remove_greater"}).anyMatch(s ->
                    ...
                    return new Command(Client.getUserUUID(),true, User.getInstance().getName(),
                ...
                return new Command(Client.getUserUUID(),false, User.getInstance().getName(), User.getInstance().getPassword(),
